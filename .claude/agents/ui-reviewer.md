---
name: ui-reviewer
description: Reviews OpenRive editor UI changes (src/components/**) for consistency with the editor's conventions (action registry, shortcuts, context menus, undo, read-only mode, theming and overflow) and verifies them in the browser. Use after implementing or changing any editor panel, tool or dialog.
...
- **Commands** live in `src/components/editor/actions.ts` with id `area.verb`, label, category, default `keys`,
...
Start the dev server (`npm run dev`) and check the change in the browser: the happy path, undo/redo, reload
