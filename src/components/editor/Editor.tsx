...
import { exportRiv, RiveDoc } from '@/lib/rive/document';
import { artboardPos, prop } from '@/lib/rive/scene';
import { downloadBytes, docStats } from '@/lib/client/projects';
import { parseDoc, stringifyDoc, toBase64 } from '@/lib/serialize';
...
import { imageSizes } from '@/lib/rive/assets';
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex min-h-0">
        <aside className="w-[240px] shrink-0 border-r border-line bg-bg1 flex flex-col">
          <div className="h-8 flex items-stretch border-b border-line">
            {(
              [
                ['layers', 'Layers', <Layers key="l" size={12} />],
                ['theme', 'Theme', <Palette key="t" size={12} />],
                ['assets', 'Assets', <FileImage key="a" size={12} />],
              ] as const
            ).map(([id, label, icon]) => (
              <button
                key={id}
                className={`flex-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold ${
                  leftTab === id ? 'text-t0 border-b-2 border-accent' : 'text-t2 hover:text-t1'
                }`}
                onClick={() => s.set('leftTab', id)}
                title={`${label} (Alt+T to switch)`}
              >
                {icon} {label}
              </button>
            ))}
