...
import { EASE_PRESETS } from '@/lib/rive/ops';
...
export function PreferencesDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center" onMouseDown={onClose}>
      <div className="bg-bg2 rounded-xl w-[460px] border border-line2" onMouseDown={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
        <div className="flex items-center p-4 border-b border-line">
          <h2 className="text-[14px] font-semibold flex-1">Preferences</h2>
