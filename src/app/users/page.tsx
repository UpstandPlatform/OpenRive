...
import { ProjectMeta, Role, ROLE_INFO, User, USER_COLORS } from '@/lib/types';
...
function DeleteDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center" onClick={onCancel}>
      <div className="bg-bg2 rounded-xl p-5 w-[400px] border border-line2" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-[14px] font-semibold mb-2">Delete {user.name}?</h2>
        ...
        {fileCount > 0 && (
          <select className="field h-8 mb-4" value={to} onChange={(e) => setTo(e.target.value)}>
            {users.map((u) => (
