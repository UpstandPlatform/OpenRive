...
import { Example, EXAMPLES } from '@openrive/rive/examples';
import { TEMPLATES, Template } from '@openrive/rive/templates';
import { ARTBOARD_PRESETS } from '@openrive/rive/presets';
...
export function NewFileDialog({ onClose, onCreate }: { onClose: () => void; onCreate: (name: string, doc: RiveDoc) => void }) {
  ...
  return (
    <Modal title="New file" width={520} onClose={onClose}>
      ...
      <div className="flex justify-end gap-2 p-4 border-t border-line">
        ...
        <button
          ...
          onClick={async () => {
            const example = EXAMPLES.find((e) => e.id === templateId);
            if (example) onCreate(name.trim() || example.name, importRiv(await loadExample(example)));
